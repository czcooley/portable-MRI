function [x,flag,relres,iter,resvec] = pcg_(A,b,tol,maxit,M1,M2,x0,varargin)
...
if (n2b == 0)                      % if    rhs vector is all zeros
    ...
    if (nargout < 2)
        itermsg_('pcg',tol,maxit,0,flag,iter,NaN);
    end
    return
end
...
r = b - iterapp_('mtimes',afun,atype,afcnstr,x,varargin{:});
...
if (normr <= tolb)                 % Initial guess is a good enough solution
    ...
    resvec = normr;
    if (nargout < 2)
        itermsg_('pcg',tol,maxit,0,flag,iter,relres);
    end
    return
end
...
resvec = gpuArray( zeros(maxit+1,1) );         % Preallocate vector for norm of residuals
...
if reconStruct_small.display == 1
    f=figure(100);
end
...
for ii = 1 : maxit
    ...
    fprintf('\tPCG iteration #%d/%d',ii,maxit);
    
    if existM1
        y = iterapp_('mldivide',m1fun,m1type,m1fcnstr,r,varargin{:});
        if ~all(isfinite(y))
            flag = 2;
            break
        end
    ...
    if existM2
        z = iterapp_('mldivide',m2fun,m2type,m2fcnstr,y,varargin{:});
        if ~all(isfinite(z))
            flag = 2;
            break
        end
    ...
    q = iterapp_('mtimes',afun,atype,afcnstr,p,varargin{:});
    pq = p' * q;
    if ((pq <= 0) || isinf(pq))
        flag = 4;
        break
    ...
    fprintf('  res = %e   time/it. = %f sec.\n',normr/n2b,TIME);
    
    if reconStruct_small.display == 1
        x_img = reshape( x,reconStruct_small.nx,reconStruct_small.ny,reconStruct_small.nz );
        
        %%%%%%%%%%%%%%%%%%%%%%
        %%changed 11/22/19
        temp = gpuArray(zeros(reconStruct_small.reconx,reconStruct_small.recony));
        temp(reconStruct_small.I) = x;
        x_img = temp;
        %%%%%%%%%%%%%%%%%%%%%%

        
        display_nimages(abs(x_img)); axis image off; colorbar; colormap gray;
        
        
        title(sprintf('PCG it. #%d',ii));
        drawnow;
    end
...
if (flag == 0)
    ...
    r_comp = b - iterapp_('mtimes',afun,atype,afcnstr,xmin,varargin{:});
...
% if (nargout < 2)
%     itermsg_('pcg',tol,maxit,ii,flag,iter,relres);
% end
