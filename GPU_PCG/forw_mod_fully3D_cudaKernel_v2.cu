						...
						int nechoes,int npix )
{
  ...
  if( index < nsamples*ncoils*nrots*nechoes*nfreqs ){
    ...
    int tmp1 = index % (nsamples*ncoils*nrots*nfreqs);
    ...
    int freq   = (index - tmp1) / (nsamples*ncoils*nrots*nfreqs);
    ...
    for( int i=0;i<npix;i++ ){
      ...
      // EXP
      float angle = -times[sample] * ( fieldmaps[i + rot*npix] + freqs[freq] );
      ...
      y_re[index] += E * sqrt(freq_weights[freq]);
      y_im[index] += F * sqrt(freq_weights[freq]);
